...
<template>
  <AuthPageFrame title="Welcome" accent="back." description="Sign in to continue your job search.">
    <form class="rounded-3xl border border-[#ded8f2] bg-white p-7 shadow-[0_24px_70px_rgba(24,37,101,0.1)] sm:p-8" @submit.prevent="login">
      ...
      <div class="mt-4 flex items-center justify-between gap-4"><label class="flex cursor-pointer items-center gap-2 text-sm text-[#5c6d99]"><input v-model="rememberMe" type="checkbox" class="size-4 accent-[#7b66ff] cursor-pointer" />Remember me</label><router-link to="/forgot-password" class="text-sm font-semibold text-[#715cdf] hover:underline">Forgot password?</router-link></div>
