  ...
  routes: [
    {
      ...
      children: [
        ...
        },
        {
          path: "saved-jobs",
          name: "SavedJobsPage",
          component: () => import("@/views/user/SavedJobsPage.vue"),
          meta: { requiresAuth: true, role: "USER" },
        },
        {
          path: "applications",
          name: "ApplicationsPage",
          component: () => import("@/views/user/ApplicationsPage.vue"),
          meta: { requiresAuth: true, role: "USER" },
        },
        {
          path: "applications/:id",
          name: "ApplicationDetailPage",
          component: () => import("@/views/user/ApplicationDetailPage.vue"),
          meta: { requiresAuth: true, role: "USER" },
        },
        {
          path: "notifications",
        ...
        },
        {
          path: "reports",
          name: "MyReportsPage",
          component: () => import("@/views/user/MyReportsPage.vue"),
          meta: { requiresAuth: true, role: "USER" },
        },
        { path: "profile/edit", redirect: "/profile?edit=1" },
        {
          path: "profile/education",
          redirect: "/profile?background=1#background",
        },
        {
          path: "profile/experience",
          redirect: "/profile?background=1#background",
        },
        { path: "profile/skills", redirect: "/profile#skills" },
        { path: "profile/resumes", redirect: "/profile#resumes" },
        {
          path: "jobs/:id/apply",
          name: "ApplicationPage",
          component: () => import("@/views/user/ApplicationPage.vue"),
          meta: { requiresAuth: true, role: "USER" },
        },
      ],
    },
    ...
    { path: "/company", redirect: { name: "CompanyDashboardPage" } },
    {
    ...
    {
      path: "/company/jobs",
      name: "CompanyJobsPage",
      component: CompanyDashboardPage,
      meta: { requiresAuth: true, role: "COMPANY" },
    },
    {
      path: "/company/applicants",
      name: "CompanyApplicantsPage",
      component: CompanyDashboardPage,
      meta: { requiresAuth: true, role: "COMPANY" },
    },
    {
      path: "/company/applicants/:id",
      name: "CompanyApplicantProfilePage",
      component: CompanyDashboardPage,
      meta: { requiresAuth: true, role: "COMPANY" },
    },
    {
      path: "/company/analytics",
      redirect: { name: "CompanyDashboardPage" },
    },
    {
...
router.beforeEach(async (to) => {
  let session;
  try {
    session = await verifySession(Boolean(to.meta.requiresAuth));
  } catch {
    // Public browsing stays available when the account service is temporarily offline.
    if (to.meta.requiresAuth)
      return {
        name: "AuthPage",
        query: { connection: "1", redirect: to.fullPath },
      };
    return true;
  }

  if (
    (to.name === "AuthPage" || to.path.startsWith("/register")) &&
    session &&
    !to.query.connection
  ) {
...
export function loginDestination(role: UserRole, redirect: unknown): string {
  ...
  if (
    ...
    if (
      ...
      !target.path.startsWith("/login") &&
      !target.path.startsWith("/auth") &&
      !target.path.startsWith("/register") &&
      !target.path.startsWith("/verify-email") &&
      (!target.meta.role || target.meta.role === "USER")
    )
