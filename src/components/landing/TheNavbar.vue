...
import { savedJobIds } from "@/services/activity";
...
let avatarLoadId = 0;
const name = computed(() =>
  currentUser.value ? displayNameForUser(currentUser.value) : "",
);
const initials = computed(() =>
  currentUser.value ? initialsForUser(currentUser.value) : "",
);
const links = [
    ...
    : isJobSeeker.value
      ? [
          { label: "View profile", to: "/profile", count: undefined },
          {
            label: "Saved jobs",
            to: "/saved-jobs",
            count: savedJobIds.value.length,
          },
      ...
      : [
          {
            label:
              currentUser.value.role === "ADMIN"
                ? "Admin dashboard"
                : "Company dashboard",
            to: dashboardPathForRole(currentUser.value.role),
            count: undefined,
          },
        ],
);
...
async function openProfileFromKeyboard(position: "first" | "last") {
  ...
  const items =
    profileMenu.value?.querySelectorAll<HTMLElement>('[role="menuitem"]');
...
function focusOut(event: FocusEvent) {
  if (
    event.relatedTarget &&
    !header.value?.contains(event.relatedTarget as Node)
  )
    closeMenus();
}
function releaseHeaderAvatar() {
...
async function loadHeaderAvatar() {
  ...
  try {
    let profile = currentCandidateProfile.value;
    if (!profile) {
      const response = await apiRequest<{ profile?: CandidateProfile }>(
        "/account/profile",
      );
      if (loadId !== avatarLoadId || currentUser.value?.id !== user.id) return;
    ...
    const blob = await apiBlob("/account/profile/avatar");
    if (loadId !== avatarLoadId || currentUser.value?.id !== user.id) return;
    headerAvatarUrl.value = URL.createObjectURL(blob);
  } catch {
    // The header should still work when the profile service is temporarily offline.
  }
}
...
watch(
  currentUser,
  (user, previous) => {
    if (user?.role === "USER") startNotificationRealtime();
    else stopNotificationRealtime();
    if (!user) {
      closeMenus();
      setCandidateProfile(null);
      setUnreadNotificationCount(0);
      releaseHeaderAvatar();
    } else if (user.id !== previous?.id) {
      setCandidateProfile(null);
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener("focus", refreshHeaderNotifications);
  releaseHeaderAvatar();
});
</script>
      ...
      <div class="header-actions">
        ...
        <template v-else>
          <router-link
            ...
            :class="{ 'is-active': route.path === '/notifications' }"
          ...
          <div class="profile-control">
            <button
              ...
              @click="openProfile"
              @keydown.down.prevent="openProfileFromKeyboard('first')"
              @keydown.up.prevent="openProfileFromKeyboard('last')"
            >
              <img
            ...
            <Transition name="dropdown">
              ...
              >
                ...
                >
                  {{ link.label
                  }}<span v-if="link.count !== undefined" class="menu-count">{{
                    link.count
                  }}</span>
                </router-link>
                <button
...
.site-header {
  ...
  border-bottom: 1px solid #e6e1fa;
...
.header-inner {
  max-width: 1440px;
  margin: auto;
  padding: 20px clamp(24px, 7.5vw, 108px);
  display: flex;
  align-items: center;
  gap: 40px;
}
...
:deep(.navbar-brand-logo.brand-logo--wordmark img) {
...
.profile-control {
  ...
  border-radius: 999px;
  background: #faf9ff;
  box-shadow: 0 2px 8px rgb(77 53 205 / 7%);
}
.profile-button {
  display: flex;
  align-items: center;
...
.account-menu {
  ...
  border-radius: 16px;
  background: white;
  box-shadow: 0 16px 48px rgb(26 30 80 / 14%);
}
.account-identity {
...
.mobile-navigation {
  ...
  background: #f9f8ff;
  border-bottom: 1px solid #e6e1fa;
  box-shadow: 0 12px 20px #0b2b8210;
}
.mobile-navigation a {
  padding: 14px 8px;
  border-radius: 8px;
